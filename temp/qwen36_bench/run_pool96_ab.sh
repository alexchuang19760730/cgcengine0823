#!/bin/bash
# Clean interleaved A/B at 96 slots: baseline vs pool64 vs pool80.
set -euo pipefail
...
run_case() {
  ...
  env "$@" \
    ...
    > "$OUT/i9_$name.log" 2>&1 || echo "$name exit=$?"
}

# 2 rounds interleaved to cancel load drift
for round in 1 2; do
  run_case "r${round}_base"    TURBO_FIELDFARE_EXPERT_SLOTS=96
  run_case "r${round}_p64"     TURBO_FIELDFARE_EXPERT_SLOTS=96 TURBO_FIELDFARE_HOT_POOL=1 TURBO_FIELDFARE_HOT_POOL_EXPERTS=64 TURBO_FIELDFARE_HOT_POOL_PROFILE=$OUT/qwen36_top64.json TURBO_FIELDFARE_HOT_POOL_PRELOAD=sync
