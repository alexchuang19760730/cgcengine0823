#!/bin/bash
# Cold-cache A/B: blocking eager pool open (EAGER_POOL_OPEN=1) vs lazy async.
# qwen36-r3 self profile, flush = read 21GB r4. Usage: run_eager_block_ab.sh <round>
set -uo pipefail
...
run_case() {
  ...
  /usr/bin/time -l env "$@" \
    ...
    > "$OUT/b${R}_$name.log" 2> "$OUT/b${R}_$name.time" || echo "$name exit=$?"
}
...
flush_cache
run_case "eager_block" TURBO_FIELDFARE_EXPERT_SLOTS=96 TURBO_FIELDFARE_HOT_POOL=1 TURBO_FIELDFARE_HOT_POOL_EXPERTS=80 TURBO_FIELDFARE_HOT_POOL_PROFILE=$P80 TURBO_FIELDFARE_HOT_POOL_PRELOAD=async TURBO_FIELDFARE_EAGER_POOL_OPEN=1
