#!/bin/bash
# §13.105: r4q4la_ga → r4q4la_ga_e4（embed_tokens 4-bit，純磁碟節省）
set -e
cd /Users/alexchuang/Documents/flashkv0516
PARENT=qwen36-r4q4la_ga
NAME=r4q4la_ga_e4
SRC=prime-agent-worktrees/$PARENT.gturbo
DST=prime-agent-worktrees/qwen36-$NAME.gturbo
TMPBIN=/tmp/r4q4la_ga_e4.bin

echo "=== [1/4] apply (already done) ==="
...
emb = e['model.language_model.embed_tokens.weight']
assert emb['dtype'] == 0 and emb['scaleSize'] > 0, 'embed not q4'
...
print(f'  embed q4 OK, packed={len(packed)} broken=0')
"

echo "=== [2/4] assemble (tokenizer 先行 + experts hardlink + mv) ==="
rm -rf "$DST"
...
ls "$DST/model_weights.bin" | awk '{print "  resident: " $5/1e6 " MB"}'
