#!/bin/bash
# §13.93 lm_head xSmem A/B: TURBO_FIELDFARE_Q36_HEAD_XSMEM 0 vs 1
# r3 生產設定 (96 slots + pool80 + sync preload + top80), msg_code, 128 tok, 交錯 2 輪
set -euo pipefail
...
run_case() {
  ...
  env $BASE_ENV TURBO_FIELDFARE_Q36_HEAD_XSMEM=$xsmem \
    ...
    > "$OUT/q36_head_$name.log" 2>&1 || echo "$name exit=$?"
  echo "done $name"
}
...
echo ALLDONE > "$OUT/q36_head.done"
