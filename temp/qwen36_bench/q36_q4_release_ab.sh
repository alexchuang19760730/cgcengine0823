#!/bin/bash
# §13.99 release 驗證: r3 vs r3q4 128 tok 交錯 2 輪 (release build + trust-receipt)
set -uo pipefail
ROOT=/Users/alexchuang/Documents/flashkv0516
BIN=$ROOT/prime-agent-worktrees/turbo-fieldfare/.build/release/TurboFieldfareCLI
...
run_case() {
  ...
  env $BASE_ENV TURBO_FIELDFARE_HOT_POOL_PROFILE="$ROOT/prime-agent-worktrees/$model/profiles/top80_code.json" \
    ...
    > "$OUT/q4rel_$name.log" 2>&1 || echo "$name exit=$?"
  echo "done $name"
}
...
echo ALLDONE > "$OUT/q36_q4rel.done"
