#!/usr/bin/env python3
"""Quick 64-token ppl sanity check on the r4 repack. Decisive correctness test:
dense + lm_head from local model_weights.bin, experts streamed from local layer files."""
...
def load_dense_state_dict(gturbo):
    # per-entry seek/read to avoid 4.9GB read_bytes() peak (OOM-safe)
...
log(f"building model ({num_layers} layers, {n_experts} experts, {BITS}-bit)...")
...
# GPU-exact token ids (from engine dump); skip tokenizer
...
# per-layer post-attn norm stats + final logits top-5 for GPU comparison
norm_stats = {}
def make_hook(i):
    def hook(module, inp, out):
        h = out[0] if isinstance(out, tuple) else out
        n = h.detach().float()  # [13,2048]
        norm_stats[i] = (
            float(n[0].std()), np.round(n[0][:8].numpy(), 3).tolist(),
            float(n[-1].std()), np.round(n[-1][:8].numpy(), 3).tolist())
    return hook
...
for i in range(num_layers):
    ...
    if st is None:
        continue
    s0, f80, s1, f81 = st
    log(f"CPU L{i}.normed pos0 std={s0:.4f} first8={f80}")
    log(f"CPU L{i}.normed pos12 std={s1:.4f} first8={f81}")
emb0 = model.model.embed_tokens(ids)[0, 0]
emb1 = model.model.embed_tokens(ids)[0, -1]
log(f"CPU embed pos0 std={float(emb0.std()):.4f} first8={np.round(emb0[:8].float().numpy(), 3).tolist()}")
log(f"CPU embed pos12 std={float(emb1.std()):.4f} first8={np.round(emb1[:8].float().numpy(), 3).tolist()}")
log("=== SANITY DONE ===")
