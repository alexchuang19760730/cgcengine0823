#!/bin/bash
# Clean-window interleaved: 128p96 vs 96p80 (2 rounds, 128 tok) to close the upper-bound decision.
set -euo pipefail
...
run_case() {
  ...
  /usr/bin/time -l env "$@" \
    ...
    > "$OUT/cl_$name.log" 2> "$OUT/cl_$name.time" || echo "$name exit=$?"
}
...
for round in 1 2; do
  ...
  run_case "r${round}_s128p96" TURBO_FIELDFARE_EXPERT_SLOTS=128 TURBO_FIELDFARE_HOT_POOL=1 TURBO_FIELDFARE_HOT_POOL_EXPERTS=96 TURBO_FIELDFARE_HOT_POOL_PROFILE=$P96 TURBO_FIELDFARE_HOT_POOL_PRELOAD=sync
done
echo done
