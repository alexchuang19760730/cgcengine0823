...
common_init_result_ptr common_init_from_params(common_params & params, bool model_only) {
    ...
    if (params.warmup) {
        ...
        if (llama_model_has_decoder(model)) {
            // [CGC §8.108] use llama_n_batch (capped by L4 pool) not params.n_batch (user value)
            llama_decode(lctx, llama_batch_get_one(tmp.data(), std::min(tmp.size(), (size_t) llama_n_batch(lctx))));
        }
...
common_context_seq_rm_type common_context_can_seq_rm(llama_context * ctx) {
    ...
    // [CGC §8.108] L4 metal pool caps n_batch to 1 (decode-only); probe must respect it
    std::vector<llama_token> tmp;
    const int n_probe = std::min(2, (int) llama_n_batch(ctx));
...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    mparams.expert_cache_bytes = params.expert_cache_bytes;
