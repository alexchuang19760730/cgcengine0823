...
extern "C" {
    ...
    // L1: per-expert offset index entry for paged expert tensors (expert streaming / bounded residency)
    struct llama_expert_index_entry {
        uint32_t layer;       // layer index (0-based); UINT32_MAX = not layer-scoped
        uint32_t expert;      // expert index within the layer
        int32_t  kind;        // llama_expert_kind: 0=gate, 1=up, 2=down, 3=gate_up (merged)
        int32_t  type;        // ggml_type of the tensor
        uint32_t file_idx;    // source GGUF file index (0 = main file)
        uint64_t file_offset; // absolute file offset of this expert's contiguous slice (data_start + relative tensor offset)
        uint64_t bytes;       // bytes of this expert's slice (= tensor stride over the expert dim)
    };
    ...
    struct llama_model_params {
        ...
        // [EXPERIMENTAL] expert streaming (bounded residency), L1:
        // if > 0, FFN expert tensors (LLM_TENSOR_FFN_*_EXPS) are marked as paged and a per-expert
        // offset index is built (see llama_model_expert_index). 0 = disabled, behavior unchanged.
        // The L2 pool / L3 gather that actually consume this index land in a later stage.
    ...
    // [EXPERIMENTAL] L1: paged-expert offset index (empty when llama_model_params.expert_cache_bytes = 0)
    LLAMA_API size_t llama_model_expert_index_size(const struct llama_model * model);
    LLAMA_API const struct llama_expert_index_entry * llama_model_expert_index(const struct llama_model * model);

    // [EXPERIMENTAL] L2: bounded resident expert cache (expert streaming)
    // Cache unit = (layer, expert); a blob holds all present kinds (gate/up/down or merged gate_up)
    // concatenated in index order. Bytes are read from the GGUF file with pread at the L1 index's
    // absolute offsets (segment-aware). Requires the model loaded with expert_cache_bytes > 0.
    struct llama_expert_cache;

    // budget_bytes = max resident bytes for expert blobs (0 = disabled, returns NULL).
    LLAMA_API struct llama_expert_cache * llama_expert_cache_init(const struct llama_model * model, size_t budget_bytes);

    // Ensure (layer, expert) pairs are resident; blocking fill of misses. Returns the number of misses filled.
    LLAMA_API size_t llama_expert_cache_ensure(struct llama_expert_cache * cache, const uint32_t * layers, const uint32_t * experts, size_t n);

    // Non-blocking: queue (layer, expert) pairs for background pread (low priority thread).
    LLAMA_API void llama_expert_cache_prefetch(struct llama_expert_cache * cache, const uint32_t * layers, const uint32_t * experts, size_t n);

    // Copy k experts of one kind (0=gate, 1=up, 2=down, 3=gate_up) into dst as [k][seg_bytes] rows
    // (dst_stride = bytes between consecutive experts, 0 = seg_bytes). Returns total bytes copied,
    // or -1 if any expert is not resident (call ensure() first).
    LLAMA_API int64_t llama_expert_cache_fill(struct llama_expert_cache * cache, uint32_t layer,
            const uint32_t * experts, size_t k, int kind, void * dst, size_t dst_stride);

    LLAMA_API void llama_expert_cache_get_stats(const struct llama_expert_cache * cache,
            size_t * requests, size_t * hits, size_t * misses, size_t * resident_bytes, size_t * file_reads);

    LLAMA_API void llama_expert_cache_free(struct llama_expert_cache * cache);
