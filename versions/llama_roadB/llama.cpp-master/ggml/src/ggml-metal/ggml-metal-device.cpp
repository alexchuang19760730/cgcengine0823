...
// CGC P1-3b: compile-time nr0 selection for the per-expert GEMV kernels (env CGC_MMV_NR0=8;
// default 4). Only the types qwen36/gemma4 MoE use are overridable.
static int ggml_metal_nr0_env(ggml_type t) {
    const char * e = getenv("CGC_MMV_NR0");
    if (e != nullptr && strcmp(e, "8") == 0) {
        if (t == GGML_TYPE_IQ2_S || t == GGML_TYPE_IQ3_XXS) {
            return 8;
        }
    }
    switch (t) {
        case GGML_TYPE_IQ2_S:   return N_R0_IQ2_S;
        case GGML_TYPE_IQ3_XXS: return N_R0_IQ3_XXS;
        default:                return -1;
    }
}
...
// nsg does not change the per-row reduction order (bit-identity holds). Clamped to the
// M4 threadgroup limit (1024 threads / 32 per simdgroup = 32).
static int ggml_metal_nsg_env(ggml_type t) {
    const char * e = getenv("CGC_MMV_NSG");
    if (e != nullptr) {
        int v = atoi(e);
        if (v >= 1 && v <= 32) {
            return v;
        }
        fprintf(stderr, "CGC_MMV_NSG: ignoring invalid value '%s' (clamp 1..32)\n", e);
    }
    switch (t) {
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    switch (tsrc0) {
        ...
        case GGML_TYPE_IQ3_XXS:
            {
                nsg = ggml_metal_nsg_env(GGML_TYPE_IQ3_XXS);
                nr0 = ggml_metal_nr0_env(GGML_TYPE_IQ3_XXS);
                // fused-down needs extra shmem for GLU[512] = 2048 bytes
                smem = (getenv("CGC_GLU_FUSED_DOWN") != nullptr) ? 256*4+128+512*4 : 256*4+128;
            } break;
        ...
        case GGML_TYPE_IQ2_S:
            {
                nsg = ggml_metal_nsg_env(GGML_TYPE_IQ2_S);
                nr0 = ggml_metal_nr0_env(GGML_TYPE_IQ2_S);
            } break;
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_id(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    if (nr0 == 8) {
        snprintf(base, 256, "kernel_mul_mv_id_%s_%s%s_nr0_8", ggml_type_name(tsrc0), ggml_type_name(tsrc1), suffix);
    }
    snprintf(name, 256, "%s_nsg=%d", base, nsg);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv_id_glu(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    switch (tsrc0) {
        ...
        default:
            GGML_ABORT("%s: unsupported gate/up type %s for fused MUL_MAT_ID_GLU phase-1 (CGC P1-3a)\n",
                    __func__, ggml_type_name(tsrc0));
    }

    snprintf(base, 256, "kernel_mul_mv_id_glu_%s_%s", ggml_type_name(tsrc0), ggml_type_name(tsrc1));
    if (nr0 == 8) {
        snprintf(base, 256, "kernel_mul_mv_id_glu_%s_%s_nr0_8", ggml_type_name(tsrc0), ggml_type_name(tsrc1));
    }
    snprintf(name, 256, "%s_nsg=%d", base, nsg);
...
// CGC §8.66: fused ADD + RMS_NORM single-dispatch kernel (two dsts: residual + norm).
// op is the ADD node; the kernel name suffix follows ne[0] % 4 like the norm family.
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_add_rms_norm(ggml_metal_library_t lib, const ggml_tensor * op) {
    assert(op->op == GGML_OP_ADD);
    ...
    snprintf(base, 256, "kernel_add_rms_norm_f32%s", suffix);
