...
## 12. State ABI v2.0 通用化升级定义

本节不替代前文 `v1.2` 的 `DeepSeek V2 -> V4` 最小 ABI 结论，而是在其之上给出面向多模型、多模态、MoE 与端云协同的通用化升级方向。其目标是把当前按模型家族硬编码的判定，升级为可注册、可组合、可代码化的行业通用 ABI 底座。

### 12.1 升级目标

`State ABI v2.0` 的目标是：

- 去除 `DeepSeek` 专属标签，改为通用架构语义标签
- 让任何模型只需注册自己的 ABI 元数据，而无需修改核心判定代码
- 把 loader、compiler、runtime、edge-cloud protocol 统一到同一套 ABI 语义契约
- 让 `KV cache`、`Output Projection`、多模态状态、MoE/Router 状态都纳入统一兼容决策

### 12.2 新增通用状态项

在 `v1.2` 的 `KVState / OutputProjectionState / TensorPhysicalSpec / CompatDecision` 基础上，`v2.0` 新增以下通用状态维度：
...
- `VisionState`：描述视觉塔、视觉 token 化与视觉-文本融合语义
- `FFNExpertState`：描述 dense FFN / shared MoE / routed MoE / hybrid FFN-MoE 结构
- `RouterState`：描述 expert router 的选择、容量、归一化与辅助损失策略
- `LayoutMeta`：描述 head dim、latent rank、group 维等结构超参数
- `CacheSchema`：描述 KV / KDA / prefix cache / resume payload 的语义版本

### 12.3 通用 Variant 枚举定义

#### 12.3.1 KV 变体

`kv_variant` 统一定义为以下枚举之一：
...
- `mha_plain`：经典多头注意力原生 KV
- `gqa_shared_head`：grouped-query attention，共享 KV head
- `latent_two_stage`：存在两段式 latent KV 生成链路
- `grouped_wkv`：原生 grouped / fused WKV consumer contract

#### 12.3.2 输出投影变体

`output_proj_variant` 统一定义为以下枚举之一：

- `single_linear`
- `two_stage_lora_grouped`

语义说明：

- `single_linear`：单段全秩输出投影
- `two_stage_lora_grouped`：带 grouped domain / low-rank / two-stage output projection

#### 12.3.3 视觉状态

`vision_state` 统一定义为以下枚举之一：

- `none`
- `single_tower_patchified`
- `dual_stream_vision_text`
- `fused_multimodal_latent`

#### 12.3.4 FFN / Expert 状态

`ffn_expert_state` 统一定义为以下枚举之一：
...
`router_state` 统一定义为以下枚举之一：

- `none`
- `topk_sigmoid`
- `topk_softmax`
- `aux_loss_router`
- `capacity_limited_router`

### 12.4 通用 ABI 描述对象

`v2.0` 推荐把模型与 checkpoint 的语义描述统一抽象为以下三层对象：
...
- 兼容判定只能依赖 `AbiDescriptor`
- 不得直接依赖模型品牌名进行硬编码特判
- `DeepSeek-V4`、`Qwen-VL`、`LLaMA-MoE` 等仅是注册名，不是兼容判定依据

### 12.5 通用 Compat 决策规则

`v2.0` 延续 `CompatDecision` 的四分类：
...
但决策输入从 `DeepSeek V2 -> V4` 特化判断，升级为通用规则：

- 若仅 `TensorPhysicalSpec / LayoutMeta` 差异，且 `AbiState` 语义不变，则允许 `loader_adaptor_ok`
- 若需要 checkpoint 载入后做有限结构转换，但不改变 forward 语义，则允许 `postload_adaptor_ok`
- 若 `kv_variant / output_proj_variant / vision_state / ffn_expert_state / router_state / cache_schema` 任一跨语义边界变化，则必须 `runtime_branch_required`
- 若缺失 runtime 必需结构，且不存在合法 branch 可承接，则为 `incompatible`

### 12.6 VariantRegistry 与插件接口

`v2.0` 要求把核心兼容系统拆为注册表与插件，而不是继续在 loader 中写死模型家族判断。
...
- `VariantRegistry`：注册模型 / checkpoint 的 `AbiDescriptor`
- `CompatRuleEngine`：根据 `source AbiDescriptor` 与 `target AbiDescriptor` 产出 `CompatDecision`
- `WeightMappingPlugin`：实现合法的 key remap / post-load 适配
- `RuntimeBranchPlugin`：实现 `runtime_branch_required` 时的前向分支

推荐实施顺序：

1. 先把当前 `DeepSeek` 路径抽象为 `AbiDescriptor + VariantRegistry + Plugin`
2. 让现有 `legacy_o_proj / legacy_kv` 成为第一版 `RuntimeBranchPlugin`
3. 再把 `_map_static_weight_key()` 抽象成第一版 `WeightMappingPlugin`
4. 最后把 `VisionState / FFNExpertState / RouterState` 接入同一套引擎

### 12.7 与 CGC 全栈的对接方式

`State ABI v2.0` 必须同时贯穿以下四层：

#### A. 加载器层

- 新模型架构只注册 variant 元数据
- 核心判定逻辑不再新增模型名 if/else
- loader adaptor 与 forbidden mapping 全部由 `CompatRuleEngine + WeightMappingPlugin` 驱动

#### B. 编译层

- `CGC Engine` 八步流水线读取 `AbiDescriptor`
- 自动匹配 layout、quant、shard、cache schema
- 编译器输入不再只是 graph，也包含 ABI 语义信息

#### C. 运行时层

- `runtime branch` 变为模块化插件
- 不同模型架构的前向路径可以插拔式接入
- `legacy_o_proj / legacy_kv` 是第一批样板分支

#### D. 端云协议层

- ABI 元数据必须成为 KV / cache / resume payload 的标准头部
- 端云只有在 ABI 头一致时才允许 zero-copy 复用
- 禁止仅凭 tensor 名称或 shape 相近就复用端云状态

### 12.8 建议的端云 ABI Header
...
- ABI header 是运行时协议的一部分，不只是调试信息
- ABI header 必须可序列化、可审计、可比较
- 任意端云 KV / prefix / resume payload 的 zero-copy 复用，必须先通过 ABI header 校验

### 12.9 Megatrain 八步流水线统一执行矩阵

`State ABI v2.0` 若要真正落到 `CGC Megatrain` 八步流水线，不能只描述模型内部状态，还必须让流水线在入口阶段显式感知任务、硬件、模型与环境上下文。当前主线建议把这组信息统一抽象为一套正式执行矩阵，并在 `step0 / step1` 之前完成检测、归档与写入 `ExecutionContext`。
...
尽管历史上常口语化称为 `4D 感知矩阵`，当前工程落地已经收敛为：
...
`task_entity` 用于标识当前流水线服务的上层任务实体，推荐枚举：
...
- `agent`：面向 Agent 工作流或工具使用任务
- `embodied`：面向机器人 / 具身控制 / 端云协同执行任务
- `model`：面向纯模型训练、推理、蒸馏或系统 smoke 任务

#### 12.9.2 硬件维

`hardware_platform` 用于标识当前任务所在的硬件平台，推荐至少覆盖：
...
`hardware_topology` 用于标识当前任务所处的物理执行拓扑，推荐至少覆盖：
...
- `single_node_cpu`
- `edge_device_single_accelerator`
- `multi_node_tp_pp_ep`

其中历史字段 `hardware_scope` 在当前实现里可视为 `hardware_topology` 的兼容别名。

#### 12.9.3 模型维

`model_scope` 用于标识当前运行使用的是云侧模型还是端侧模型，推荐至少覆盖：

- `edge_model`
- `cloud_model`

`model_assembly` 用于标识当前运行使用的是哪种模型装配层级，推荐至少覆盖：
...
- `none_minimal_collective`：完全脱离模型语义，仅验证 distributed collective
- `tiny`：最小可运行结构，用于功能烟雾测试
- `real_weights`：真实模型结构与真实 checkpoint 权重

#### 12.9.4 环境维

`environment` 用于标识部署环境语义，推荐至少覆盖：
...
`runtime_mode` 沿用当前八步流水线已存在的四态定义：
...
统一执行矩阵与 `State ABI` 的关系是：

- `State ABI` 描述模型与状态语义是否兼容
- 统一执行矩阵描述当前任务在哪种任务实体、模型侧、硬件平台、硬件拓扑、环境与运行模式下执行
...
- compiler 策略选择
- runtime branch 选择
- cache / zero-copy / distributed collective 策略选择

一句话说：

- `ABI` 决定“能不能兼容”
- 统一执行矩阵决定“应该以什么执行策略兼容”

#### 12.9.7 三层正式配置分工

为了让 `State ABI` 与 runtime contract、profile setting、system manifest 的职责边界不再混淆，当前主线建议把正式配置分成三层：
...
- `Profile Setting + Binding`
  - 当前任务应该怎么跑
- `System Profile`
  - 当前系统由谁组成、实例数是多少、哪些组件是正式 required variable

这意味着：

- `State ABI` 不应该去承担系统拓扑真源职责
- `System Profile` 也不应该伪装成 ABI 语义字段
- `Profile Binding` 更不应该替代系统拓扑定义

#### 12.9.8 State ABI 与 System Profile 的边界

`State ABI` 与 `System Profile` 的关系必须明确划开：

- `State ABI`
  - 描述模型内部状态语义
  - 例如 `kv_variant / output_proj_variant / vision_state / ffn_expert_state / router_state / cache_schema`
- `System Profile`
  - 描述系统外部编组语义
  - 例如 `DeepSeek-V4-Flash x4 + MiniCPM5 x1 + FusionRoute x1`

二者关系可以理解为：

- `State ABI`
  - 回答“这个模型状态在语义上能不能被另一条 runtime branch 正确消费”
- `System Profile`
  - 回答“当前系统到底是不是由这些组件以这种拓扑正式组成”

因此，以下信息应进入 `system_profile`，而不是进入 ABI 主体：
...
- `system_execution_manifest.json`
  - 持有完整 `system_profile`
- `runtime_contract.json / runtime_protocol_contract`
  - 只持有 `system_profile_ref` 或摘要
- `State ABI`
  - 继续只负责语义兼容，不按品牌名或系统名硬编码特判

一句话说：

- `System Profile` 决定“系统是谁”
- `Execution Matrix` 决定“这次怎么跑”
- `State ABI` 决定“语义能不能兼容”

### 12.10 与 v1.2 的关系

`v1.2` 与 `v2.0` 的关系是：

- `v1.2` 负责把当前 `DeepSeek V2 -> V4` 的伪兼容拆穿
- `v1.2` 负责逼出 `legacy_o_proj / legacy_kv` 这类真实 runtime branch
- `v2.0` 负责把上述经验抽象成通用平台能力

一句话说：

- `v1.2` 解决“当前不该假装兼容的问题”
- `v2.0` 解决“未来如何把兼容做成通用底座的问题”

## 13. 一句话总结

本 `State ABI` 白皮书在 `v1.2` 中明确了 `DeepSeek-V2 -> V4` 的最小语义 ABI 与 `legacy_o_proj / legacy_kv` patch 路径，在 `v2.0` 中进一步给出通用化升级方向：以 `AbiDescriptor + VariantRegistry + CompatRuleEngine + Plugin` 为核心，把模型兼容从单模型修补升级为可扩展的行业通用 ABI 底座。

`CGC Engine` 与端云零拷贝 KV 只能在语义一致的前提下进行物理层优化，无法替代模型结构层面的显式兼容分支；未来的 zero-copy 复用必须以 ABI header 对齐为前提，而不是依赖名称或 shape 的近似匹配。
