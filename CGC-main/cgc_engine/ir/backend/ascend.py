...
class AscendBackend(Backend):
    ...
    def _run_function(self, func: Dict[str, Any], inputs: Dict[str, Any]) -> Dict[str, Any]:
        return {"status": "success", "message": f"Function {func['name']} would run on Ascend CANN"}
    ...
    def optimize(self, module: CGCModule, level: int = 3) -> CGCModule:
        ...
        if level >= 2:
            ...
            module = self._convert_to_flash_attention(module)
        if level >= 3:
    ...
    def _convert_to_flash_attention(self, module: CGCModule) -> CGCModule:
        for func in module.functions.values():
            new_body = []
            for node in func.body:
                if node.op_type == "Attention":
                    flash_attn_node = CGCNode(
                        op_type="FlashAttention",
                        name=f"{node.name}_flash",
                        inputs=node.inputs,
                        outputs=node.outputs,
                        attributes={**node.attributes, "backend": "ascend"}
                    )
                    new_body.append(flash_attn_node)
                else:
                    new_body.append(node)
            func.body = new_body
        return module
