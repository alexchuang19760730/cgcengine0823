// test_real_qwen35moe.c — 用真實 llama.cpp qwen35moe GGUF 驗證 expert streaming 定址
//
// llama.cpp qwen35moe（IQ2/IQ3）: 每層 3 個獨立 packed 張量
//   blk.N.ffn_gate_exps / ffn_up_exps / ffn_down_exps，各 [out, in, n_experts]，
//   專家 e 的權重分散在三個非連續檔案區段（gate/up 用 IQ2_XXS、down 用 IQ2_S）。
// 本工具驗證 cgc_expert_segments() 產生的 3 段 offset/size：
//   1. 與 GGUF tensor 表的 offset/type 一致
//   2. 對多個 expert（含邊界 0/3/255）pread 到的 bytes，與「tensor 表直接定址」
...
int main(int argc, char** argv) {
    ...
    if (layout.has_segments) {
        ...
        if (layout.seg_size[0] + layout.seg_size[1] + layout.seg_size[2] != layout.expert_stride) {
            fprintf(stderr, "  [FAIL] seg_size sum != expert_stride\\n");
            return 1;
        }
    }

    // --- 對照: GGUF tensor 表（層 0 三張量）---
    cgc_gguf_lite_ctx_t* ctx = cgc_gguf_lite_load(path);
    if (!ctx) { fprintf(stderr, "gguf load failed\\n"); return 1; }

    uint64_t t_off[3] = {0, 0, 0};
    int64_t  t_ne[3] = {0, 0, 0};
    int      t_type[3] = {-1, -1, -1};
    const char* t_suffix[3] = { ".ffn_gate_exps.", ".ffn_up_exps.", ".ffn_down_exps." };
    for (uint64_t i = 0; i < ctx->n_tensors; i++) {
        const char* n = ctx->tensor_names[i];
        if (n[0] != 'b' || n[1] != 'l' || n[2] != 'k' || n[3] != '.' || n[4] != '0' || n[5] != '.') continue;
        for (int k = 0; k < 3; k++) {
            if (strstr(n, t_suffix[k])) {
    ...
    for (int k = 0; k < 3; k++) {
        ...
        if (!layout.has_segments) {
            fprintf(stderr, "  [FAIL] expected has_segments=1 for llama.cpp 3-tensor layout\\n");
            fail = 1;
    ...
    printf(fail ? "\\nRESULT: FAIL\\n" : "\\nRESULT: PASS — llama.cpp GGUF expert streaming addressing verified\\n");
    return fail ? 1 : 0;
}
