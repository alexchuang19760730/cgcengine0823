#!/usr/bin/env python3
"""
Native vLLM vs FlashKDA CUDA 內核 完整對比 Benchmark

架構:
  Native vLLM:    標準 FlashAttention (O(N^2) softmax attention)
  FlashKDA:       MoonshotAI CUDA 內核實現的 KDA (O(N) 線性注意力)
"""
import os
import sys
import json
...
def benchmark_flashkda():
    """
    FlashKDA CUDA 內核 Benchmark
    """
    print("\n" + "=" * 100)
    print("  FlashKDA (MoonshotAI CUDA 內核)")
    print("=" * 100)
    ...
    for ctx_len in CONTEXT_LENGTHS:
        ...
        # Warmup
        for _ in range(3):
            flash_kda.fwd(q, k, v, g, beta, scale, out, A_log, dt_bias,
        ...
        g_dec = torch.ones(B, 1, H, K, device='cuda', dtype=torch.bfloat16)
        beta_dec = torch.zeros(B, 1, H, device='cuda', dtype=torch.bfloat16)
        out_dec = torch.empty(B, 1, H, V, device='cuda', dtype=torch.bfloat16)

        T_total_dec = B * 1
        workspace_size_dec = flash_kda.get_workspace_size(T_total_dec, H, B)
        workspace_dec = torch.empty(workspace_size_dec, device='cuda', dtype=torch.uint8)
        ...
        for _ in range(OUTPUT_LENGTH):
            flash_kda.fwd(q_dec, k_dec, v_dec, g_dec, beta_dec, scale,
                           out_dec, A_log, dt_bias, lower_bound, None, None, None)
        end_event.record()
        torch.cuda.synchronize()
        ...
        del q, k, v, g, beta, out, q_dec, k_dec, v_dec, g_dec, beta_dec, out_dec
        del workspace, workspace_dec
        clear_gpu()
...
def print_comparison(native: List[Dict], flashkda: List[Dict]):
    print("\n")
    print("=" * 160)
    print("  Native vLLM vs FlashKDA (MoonshotAI CUDA 內核) 對比")
    print("=" * 160)

    header = (
        f"{'Ctx':<6} | "
        f"{'Native-Prefill':<14} {'Native-Decode':<14} {'N-GPU':<8} | "
        f"{'FlashKDA-Prefill':<16} {'FlashKDA-Decode':<16} {'F-GPU':<8} | "
        f"{'PF-Δ':<10} {'DC-Δ':<10}"
    )
    print(header)
    print("-" * 160)
    ...
    for n, f in zip(native, flashkda):
        ...
        line = (
            ...
            f"{f['prefill_tps']:>14,.1f}  {f['decode_tps']:>14,.1f}  {f['mem_used_gb']:>6.2f} | "
    ...
    if count > 0:
        ...
        print(f"    - FlashKDA: MoonshotAI CUDA 內核 (O(N) 線性注意力)")
        print(f"    - FlashKDA 僅測試注意力層，非端到端模型")
...
def main():
    ...
    flashkda_results = benchmark_flashkda()

    print_comparison(native_results, flashkda_results)

    out_file = "/home/gs01/benchmark_native_vs_flashkda.json"
    with open(out_file, "w") as f:
        json.dump({
            "config": {
                "native_model": "Qwen2.5-7B-Instruct",
                "flashkda": "MoonshotAI FlashKDA CUDA kernel",
                "context_lengths": CONTEXT_LENGTHS,
                "output_length": OUTPUT_LENGTH,
                "batch_size": BATCH_SIZE,
            },
            "native_vllm": native_results,
            "flashkda": flashkda_results,
