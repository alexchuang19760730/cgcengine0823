...
GATE10_MAP = GATE10_DIR / "CGC_Gate_1.0_edge_cloud_autonomy_gate_map.json"
GATE10_README = GATE10_DIR / "README.md"
...
def main() -> int:
    if not GATE10_MAP.is_file():
        return fail(f"gate_map not found: {GATE10_MAP}")
    if not GATE10_README.is_file():
        return fail(f"README not found: {GATE10_README}")

    gate_map = load_json(GATE10_MAP)
    ...
    legacy_sources = legacy_mapping.get("legacy_sources")
    if not isinstance(legacy_sources, list) or not legacy_sources:
        return fail("legacy_capability_mapping.legacy_sources is missing or empty")
    ...
    readme = GATE10_README.read_text(encoding="utf-8")
    if "## Legacy ID 映射" not in readme:
        return fail("README is missing '## Legacy ID 映射' section")

    missing_from_readme = [
        entry["legacy_capability_id"]
        for entry in entries
        if entry["legacy_capability_id"] not in readme
    ]
    ...
    print(f"legacy_source_count={len(legacy_sources)}")
    print(f"readme_path={GATE10_README}")
    print(f"gate_map_path={GATE10_MAP}")
    return 0
