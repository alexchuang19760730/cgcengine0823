#!/usr/bin/env python3
"""
Run Expert Streaming E2E Tests
测试 GGUF expert streaming 逻辑
"""
...
# 添加 expert_streaming 路径
sys.path.insert(0, str(Path(__file__).parent.parent / "cgc" / "expert_streaming" / "tests"))
...
class PythonExpertStreamer:
    ...
    def __init__(self, gguf_path: str):
        ...
        self.ggml_type = GGML_TYPE_IQ3_M if self.header["kv"].get("gemma4.quantization", "BF16") != "BF16" else GGML_TYPE_BF16
...
def run_tests():
    tests_passed = 0
    tests_failed = 0
    base_dir = os.path.join(os.environ.get("TEMP", os.getcwd()), ".tmp_test")
    ...
    # T6: Gemma 4 26B-A4B 实际维度
    def t6_gemma4_dims():
        REAL_HIDDEN = 3584
        REAL_INTER = 14336
        SCALE = 8
        hidden = REAL_HIDDEN // SCALE
        inter = REAL_INTER // SCALE
        num_experts = 8
    ...
    # 运行
    print("=" * 70)
    print("EXPERT STREAMING E2E TESTS")
    print("=" * 70)
