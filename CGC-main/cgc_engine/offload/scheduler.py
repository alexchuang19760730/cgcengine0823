...
from cgc_engine.config import CompileConfig
from cgc_engine.utils import magi_logger


def _get_unified_io_controller():
    try:
        from cgc_engine.io_unified import get_unified_io_controller
        return get_unified_io_controller()
    except ImportError:
        return None
...
class BaseScheduler(OffloadScheduler):
    ...
    def __init__(
        ...
        self.io_controller = _get_unified_io_controller()
                ...
                if is_next_iter:
                    if w_name in buffers:
                        if self.io_controller is not None:
                            loaded = self.io_controller.load_weight(
                                path=w_name,
                                shape=list(buffers[w_name].shape),
                                dtype=buffers[w_name].dtype,
                            )
                            submod_0_next_iter_weights[w_node] = loaded.to("cuda", non_blocking=True)
                        else:
                    ...
                    if self.io_controller is not None:
                        loaded = self.io_controller.load_weight(
                            path=w_name,
                            shape=list(env[w_node].shape),
                            dtype=env[w_node].dtype,
                        )
                        env[w_node] = loaded.to("cuda", non_blocking=True)
                    else:
