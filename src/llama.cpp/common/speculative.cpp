        ...
        if (!is_mem_shared) {
            ...
            // TODO:this is generally true, but would be nice to assert it
            {
                ...
                for (int i = 0; i < n_tokens - 1; i++) {
                    const float * h_row = llama_get_embeddings_nextn_ith(ctx_tgt, i);
                    if (h_row != nullptr) {
                        std::memcpy(batch.embd + (size_t)(i + 1) * n_embd, h_row, row_bytes);
                    } else {
                        memset(batch.embd + (size_t)(i + 1) * n_embd, 0, row_bytes);
                    }
                }
                if (getenv("CGC_MTP_DBG")) {
                    double nr = 0.0;
                    for (int k = 0; k < n_embd; ++k) nr += (double) batch.embd[n_embd]*batch.embd[n_embd + k];
