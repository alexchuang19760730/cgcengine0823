...
// CGC §8.66 E2E experiment: fused ADD + RMS_NORM (single dispatch, two dsts).
// Computes s = a + b; writes both the residual (dst2 = s) and rms_norm(s) (dst).
// Reduction order mirrors kernel_rms_norm_fuse_impl exactly (same threadgroup/simd
// structure, same sumf accumulation) so the norm output is bit-identical to the
// unfused ADD-then-RMS_NORM path; the add output is bit-identical to the unfused
// ADD kernel (IEEE elementwise add). src1 strides use kargs index [0] (b).
template <typename T>
kernel void kernel_add_rms_norm_fuse_impl(
        constant ggml_metal_kargs_norm & args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const T * a = (device const T *) (src0 + i03*args.nbf3[0] + i02*args.nbf2[0] + i01*args.nbf1[0]);
    device const T * b = (device const T *) (src1 + (i03%args.nef3[0])*args.nbf3[0] + (i02%args.nef2[0])*args.nbf2[0] + (i01%args.nef1[0])*args.nbf1[0]);

    float sumf = 0.0f;

    // parallel sum over (a + b)^2 -- same iteration order as kernel_rms_norm_fuse_impl
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        sumf += dot(a[i00] + b[i00], a[i00] + b[i00]);
    }
    sumf = simd_sum(sumf);
    ...
    device T * y2 = (device T *) (dst2 + i03*args.nb3 + i02*args.nb2 + i01*args.nb1);
    device T * y  = (device T *) (dst  + i03*args.nb3 + i02*args.nb2 + i01*args.nb1);
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        const T s = a[i00] + b[i00];
        y2[i00] = s;
        y[i00]  = s * scale;
    }
}

typedef decltype(kernel_add_rms_norm_fuse_impl<float>) kernel_add_rms_norm_fuse_t;

template [[host_name("kernel_add_rms_norm_f32")]]   kernel kernel_add_rms_norm_fuse_t kernel_add_rms_norm_fuse_impl<float>;
template [[host_name("kernel_add_rms_norm_f32_4")]] kernel kernel_add_rms_norm_fuse_t kernel_add_rms_norm_fuse_impl<float4>;
...
// ==================== CGC P1-3a: fused MoE FFN (MUL_MAT_ID_GLU) ====================
// Phase 1: per (expert-slot, token), compute gate and up dot products against the SAME y
// load, then write glu = silu(gate)*up to the scratch buffer [n_ff, n_expert_used, n_tokens].
// Mirrors kernel_mul_mv_iq3_xxs_f32_impl accumulation order exactly (bit-identical to the
// unfused gate+up mmid pair + swiglu for the f32 path).
template<int nr0, typename args_t>
void kernel_mul_mv_id_glu_iq3_xxs_impl(
        args_t args,
        device const char * src0,     // gate weights (expert selected via ids)
        device const char * src0u,    // up   weights
        device const char * src1,     // y
        device       char * dst,      // glu scratch or final output (if fused-down)
        device const char * ids,      // remap / slot ids
        device const char * ids_orig, // original ids (scale gather)
        device const char * scale_g,  // per-expert gate scale (f32) or null
        device const char * scale_u,  // per-expert up   scale (f32) or null
        device const char * src0d,    // down weights (fused-down path only)
        threadgroup  char * shmem,
        uint3  tgpig,
        ushort tiitg,
        ushort tiisg,
    ...
    const int iid1 = im/args.nei0;
    const int idx  = im%args.nei0;

    const int32_t i02   = ((device const int32_t *) (ids + iid1*args.nbi1))[idx];
    const int32_t e_orig = ((device const int32_t *) (ids_orig + iid1*args.nbi1o))[idx];

    const int first_row = (r0 * NSG + sgitg) * nr0;

    const uint64_t offset0  = first_row*args.nb01  + (uint64_t) i02*args.nb02;
    const uint64_t offset0u = first_row*args.nb01u + (uint64_t) i02*args.nb02u;
    const uint64_t offset1  =        r1*args.nb11  + (uint64_t) iid1*args.nb12;

    device const block_iq3_xxs * x  = (device const block_iq3_xxs *) (src0  + offset0);
    device const block_iq3_xxs * xu = (device const block_iq3_xxs *) (src0u + offset0u);
    device const float         * y  = (device const float         *) (src1  + offset1);
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        device const block_iq3_xxs * xru  = xu + ibl;
        device const uint8_t  * q3u  = xru->qs  + 8 * ib;
        device const uint16_t * gasu = (device const uint16_t *)(xru->qs + QK_K/4) + 2 * ib;
        device const half * dhu = &xru->d;

        for (short row = 0; row < nr0; row++) {
            // gate dot product (identical accumulation to kernel_mul_mv_iq3_xxs_f32_impl)
            {
                const float db = dh[0];
                const uint32_t aux32 = gas[0] | (gas[1] << 16);
                const float d = db * (0.5f + (aux32 >> 28));
                ...
                sumf_g[row] += d * (sum[0] + sum[1]);

                dh  += args.nb01/2;
                q3  += args.nb01;
                gas += args.nb01/2;
            }
            // up dot product against the SAME yl
            {
                const float db = dhu[0];
                const uint32_t aux32 = gasu[0] | (gasu[1] << 16);
                const float d = db * (0.5f + (aux32 >> 28));
    ...
    // GLU output per row: silu(gate) * up
    // If fused-down enabled (ne02d > 0), accumulate down matmul instead of writing scratch
    const bool fused_down = args.ne02d > 0;

    if (!fused_down) {
        // Original path: write GLU scratch [n_ff, n_experts, n_tokens]
        device float * dst_f32 = (device float *) dst + (uint64_t)iid1*args.ne1*args.ne0 + (uint64_t)idx*args.ne0;
        for (int row = 0; row < nr0 && first_row + row < args.ne0; ++row) {
            float g = simd_sum(sumf_g[row]) * 0.5f;
            float u = simd_sum(sumf_u[row]) * 0.5f;
            if (args.has_g_scale) g *= ((device const float *) scale_g)[e_orig];
            if (args.has_u_scale) u *= ((device const float *) scale_u)[e_orig];
            const float silu = g / (1.0f + exp(-g));
            if (tiisg == 0) dst_f32[first_row + row] = silu * u;
        }
    } else {
        // Fused-down path (fixed): write GLU to shmem, then K-loop dequant down x GLU.
        // Each thread computes ONE output dimension. Thread 0 -> d_out=0, thread 1 -> d_out=1, etc.
        // No simd_sum needed (each thread is independent).

        const int K    = args.ne00d;    // 512 (n_ff)
        const int Nout = args.ne01d;    // 2048 (n_embd)
        const int n_e  = args.ne02d;    // 8 (n_expert)
        const uint64_t nb_row_d = args.nb01d;
        const uint64_t nb_exp_d = args.nb02d;
        device float * dst_out = (device float *) dst;

        // Step 1: Compute GLU and write to shmem[512] (after svalues[1024] + ssigns[128])
        threadgroup float * glu_shmem = (threadgroup float *)(shmem + 1024 + 128);
        for (int row = 0; row < nr0 && first_row + row < args.ne0; ++row) {
            float g = simd_sum(sumf_g[row]) * 0.5f;
            float u = simd_sum(sumf_u[row]) * 0.5f;
            if (args.has_g_scale) g *= ((device const float *) scale_g)[e_orig];
            if (args.has_u_scale) u *= ((device const float *) scale_u)[e_orig];
            const float silu = g / (1.0f + exp(-g));
            if (tiisg == 0) glu_shmem[first_row + row] = silu * u;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Step 2: Each thread computes one output dimension.
        // d_out = thread index within threadgroup (0..31)
        const int d_out = tiitg;
        if (d_out < Nout) {
            float accum = 0.0f;
            const int ib32 = d_out / 32;  // which IQ3 block (0..63 for Nout=2048)
            for (int k = 0; k < K; ++k) {
                const float gl = glu_shmem[k];
                // Dequant down weight[k, d_out, expert_i02]
                device const block_iq3_xxs * db = (device const block_iq3_xxs *)(
                    src0d + (uint64_t)k * nb_row_d + (uint64_t)i02 * nb_exp_d);
                device const uint8_t  * q3d  = db->qs + 8 * ib32;
                device const uint16_t * gasd = (device const uint16_t *)(db->qs + QK_K/4) + 2 * ib32;
                const float dbv = db->d;
                const uint32_t aux32d = gasd[0] | (gasd[1] << 16);
                const float dd = dbv * (0.5f + (aux32d >> 28));

                float2 sum_d = {0};
                for (short l = 0; l < 4; ++l) {
                    const threadgroup uint8_t * g1 = (const threadgroup uint8_t *)(svalues + q3d[2*l+0]);
                    const threadgroup uint8_t * g2 = (const threadgroup uint8_t *)(svalues + q3d[2*l+1]);
                    const uint8_t signs_d = ssigns[(aux32d >> 7*l) & 127];
                    for (short j = 0; j < 4; ++j) {
                        sum_d[0] += yl[8*l + j + 0] * g1[j] * (signs_d & kmask_iq2xs[j+0] ? -1.f : 1.f);
                        sum_d[1] += yl[8*l + j + 4] * g2[j] * (signs_d & kmask_iq2xs[j+4] ? -1.f : 1.f);
                    }
                }
                accum += gl * dd * (sum_d[0] + sum_d[1]);
            }
            // Write to dst: layout [N_out, n_experts, n_tokens]
            const uint64_t out_idx = (uint64_t)d_out * n_e * args.nei1 + (uint64_t)idx * args.nei1 + iid1;
            dst_out[out_idx] = accum * 0.5f;
        }
    }
}

[[host_name("kernel_mul_mv_id_glu_iq3_xxs_f32")]]
kernel void kernel_mul_mv_id_glu_iq3_xxs_f32(
        constant ggml_metal_kargs_mul_mv_id_glu & args,
        device const char * src0,
        device const char * src0u,
        device const char * src1,
        device       char * dst,
        device const char * ids,
        device const char * ids_orig,
        device const char * scale_g,
        device const char * scale_u,
    ...
    kernel_mul_mv_id_glu_iq3_xxs_impl<N_R0_IQ3_XXS, constant ggml_metal_kargs_mul_mv_id_glu &>(args, src0, src0u, src1, dst, ids, ids_orig, scale_g, scale_u, src0d, shmem, tgpig, tiitg, tiisg, sgitg);
}

// CGC P1-3b: nr0=8 compile-time variant of the fused GLU kernel (env CGC_MMV_NR0=8).
[[host_name("kernel_mul_mv_id_glu_iq3_xxs_f32_nr0_8")]]
kernel void kernel_mul_mv_id_glu_iq3_xxs_f32_nr0_8(
        constant ggml_metal_kargs_mul_mv_id_glu & args,
        device const char * src0,
        device const char * src0u,
        device const char * src1,
    ...
    kernel_mul_mv_id_glu_iq3_xxs_impl<8, constant ggml_metal_kargs_mul_mv_id_glu &>(args, src0, src0u, src1, dst, ids, ids_orig, scale_g, scale_u, src0d, shmem, tgpig, tiitg, tiisg, sgitg);
}

// CGC P1-3a: IQ2_S fused gate+up+GLU (original UD files' gate/up type). Mirrors
// kernel_mul_mv_iq2_s_f32_impl accumulation order exactly; dst write uses 0.25f.
template<int nr0, typename args_t>
void kernel_mul_mv_id_glu_iq2_s_impl(
        args_t args,
        device const char * src0,
        device const char * src0u,
        device const char * src1,
    ...
    device const block_iq2_s * x  = (device const block_iq2_s *) (src0  + offset0);
    device const block_iq2_s * xu = (device const block_iq2_s *) (src0u + offset0u);
    device const float       * y  = (device const float       *) (src1  + offset1);
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        device const block_iq2_s * xru  = xu + ibl;
        device const uint8_t * qsu = xru->qs     + 4 * ib;
        device const uint8_t * qhu = xru->qh     + ib;
        device const uint8_t * scu = xru->scales + ib;
        device const uint8_t * signsu = qsu + QK_K/8;
        device const half * dhu = &xru->d;

        for (short row = 0; row < nr0; row++) {
            // gate dot product (identical accumulation to kernel_mul_mv_iq2_s_f32_impl)
            {
                const float db = dh[0];
                const float d1 = db * (0.5f + (sc[0] & 0xf));
                const float d2 = db * (0.5f + (sc[0] >>  4));
            ...
            {
                const float db = dhu[0];
                const float d1 = db * (0.5f + (scu[0] & 0xf));
                const float d2 = db * (0.5f + (scu[0] >>  4));

                float2 sum = {0};
                for (short l = 0; l < 2; ++l) {
                    constant uint8_t * grid1 = (constant uint8_t *)(iq2s_grid + (qsu[l+0] | ((qhu[0] << (8-2*l)) & 0x300)));
                    constant uint8_t * grid2 = (constant uint8_t *)(iq2s_grid + (qsu[l+2] | ((qhu[0] << (4-2*l)) & 0x300)));
                    for (short j = 0; j < 8; ++j) {
                        sum[0] += yl[8*l + j +  0] * grid1[j] * select(1, -1, signsu[l+0] & kmask_iq2xs[j]);
                        sum[1] += yl[8*l + j + 16] * grid2[j] * select(1, -1, signsu[l+2] & kmask_iq2xs[j]);
                    }
                }
                sumf_u[row] += d1 * sum[0] + d2 * sum[1];
    ...
    for (int row = 0; row < nr0 && first_row + row < args.ne0; ++row) {
        float g = simd_sum(sumf_g[row]) * 0.25f;
        float u = simd_sum(sumf_u[row]) * 0.25f;
        if (args.has_g_scale) {
            g *= ((device const float *) scale_g)[e_orig];
        }
        if (args.has_u_scale) {
            u *= ((device const float *) scale_u)[e_orig];
        }
        const float silu = g / (1.0f + exp(-g));
        if (tiisg == 0) {
            dst_f32[first_row + row] = silu * u;
        }
    }
}

[[host_name("kernel_mul_mv_id_glu_iq2_s_f32")]]
kernel void kernel_mul_mv_id_glu_iq2_s_f32(
        constant ggml_metal_kargs_mul_mv_id_glu & args,
        device const char * src0,
        device const char * src0u,
        device const char * src1,
    ...
    kernel_mul_mv_id_glu_iq2_s_impl<N_R0_IQ2_S, constant ggml_metal_kargs_mul_mv_id_glu &>(args, src0, src0u, src1, dst, ids, ids_orig, scale_g, scale_u, shmem, tgpig, tiitg, tiisg, sgitg);
}

// CGC P1-3b: nr0=8 compile-time variant of the fused GLU iq2_s kernel (env CGC_MMV_NR0=8).
[[host_name("kernel_mul_mv_id_glu_iq2_s_f32_nr0_8")]]
kernel void kernel_mul_mv_id_glu_iq2_s_f32_nr0_8(
        constant ggml_metal_kargs_mul_mv_id_glu & args,
        device const char * src0,
        device const char * src0u,
        device const char * src1,
    ...
    kernel_mul_mv_id_glu_iq2_s_impl<8, constant ggml_metal_kargs_mul_mv_id_glu &>(args, src0, src0u, src1, dst, ids, ids_orig, scale_g, scale_u, shmem, tgpig, tiitg, tiisg, sgitg);
}

// CGC P1-3a: down-scale pass — dst[row, slot, tok] *= scale_d[orig_ids[slot, tok]].
// Mirrors build_lora_mm_id's ggml_mul(scale) after the down mmid (f32 multiply, bit-exact).
kernel void kernel_mul_mv_id_glu_down_scale_f32(
        constant ggml_metal_kargs_mul_mv_id_glu_scale & args,
        device       char * dst,
        device const char * ids_orig,
        device const char * scale_d,
        uint tpitg[[thread_position_in_threadgroup]],
        uint ntg[[threads_per_threadgroup]]) {
    const int64_t total = (int64_t) args.ne0 * args.ne1 * args.ne2;
    device float * dst_f = (device float *) dst;
    for (int64_t i = tpitg; i < total; i += ntg) {
        const int64_t row  = i % args.ne0;
        const int64_t slot = (i / args.ne0) % args.ne1;
        const int64_t tok  = i / ((int64_t) args.ne0 * args.ne1);
        const int32_t e = ((device const int32_t *) (ids_orig + tok*args.nbi1))[slot];
        dst_f[i] *= ((device const float *) scale_d)[e];
    }
}
...
template [[host_name("kernel_mul_mv_id_iq3_xxs_f32_nr0_8")]] kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_iq3_xxs_f32_impl<8>>>;
...
template [[host_name("kernel_mul_mv_id_iq2_s_f32_nr0_8")]]   kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_iq2_s_f32_impl  <8>>>;
