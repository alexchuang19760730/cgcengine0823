...
struct test_case {
    ...
    bool eval_perf(ggml_backend_t backend, const char * op_names_filter, printer * output_printer) {
        ...
        // duplicate the op (unless the whole graph is the measured unit)
        if (!run_whole_graph()) {
        ...
        do {
            ...
            total_mem += mem;
            total_runs += run_whole_graph() ? 1 : n_runs;
...
// CGC decisive experiment (§8.65 bubble hypothesis): does a serial chain of
// dependent small ops cost more than the sum of isolated times on Metal?
// The real qwen36 decode graph runs ~2000 small kernels on the M=1 critical
// path with ~38ms unaccounted "bubble" (73ms GPU exec-sum - 35ms isolated sum).
// Candidate mechanisms: (a) per-op dependency latency on GPU, (b) dispatch
// throughput. serial chains force strict dependencies (no overlap); parallel
// fanout keeps the same kernel count but all ops read the same input (GPU can
// overlap/pipeline). op_size() is overridden to keep n_runs small so the whole
// chain -- not the duplicated final node -- is the measured unit.
struct test_op_chain : public test_case {
    const ggml_type type;
    const std::array<int64_t, 4> ne;   // decode shape [2048, 1, 1, 1]
    const int n_ops;                   // number of ops in the graph
    const int mode;                    // 0: rms serial chain; 1: rms parallel fanout; 2: rms+add alternating serial

    std::string vars() override {
        return VARS_TO_STR4(type, ne, n_ops, mode);
    }
    ...
    test_op_chain(ggml_type type = GGML_TYPE_F32,
            std::array<int64_t, 4> ne = {2048, 1, 1, 1},
            int n_ops = 16,
            int mode = 0)
        : type(type), ne(ne), n_ops(n_ops), mode(mode) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        ggml_tensor * x = ggml_new_tensor_4d(ctx, type, ne[0], ne[1], ne[2], ne[3]);
        ggml_set_name(x, "x");

        ggml_tensor * bias = ggml_new_tensor_4d(ctx, type, ne[0], ne[1], ne[2], ne[3]);
        ggml_set_name(bias, "bias");

        ggml_tensor * cur = x;
        for (int i = 0; i < n_ops; ++i) {
            if (mode == 2 && (i % 2) == 1) {
                cur = ggml_add(ctx, cur, bias); // dependent: adds previous rms output
                ggml_set_name(cur, "add");
            } else {
                cur = ggml_rms_norm(ctx, (mode == 1) ? x : cur, 1e-6f);
                ggml_set_name(cur, "rms");
            }
        }
        ggml_tensor * out = cur;
        ggml_set_name(out, "out");
    ...
    // Force n_runs small so the whole chain is the measured unit:
    // n_runs = min(graph_size - n_nodes, target_size/op_size) + 1
    // target_size (GPU) = 32 GiB; op_size = 8 GiB -> n_runs = 5 for 16-op chains.
    size_t op_size(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return (size_t) 8ULL << 30;
    }
};

// CGC P1-B (S8.65/8.70): large-kernel (MUL_MAT GEMV) dependency-wait measurement.
// mode 3: serial dependent GEMV chain (y_i = W_i * y_{i-1}, M=1) -- mirrors the decode
//   per-layer large-kernel chain (attn proj -> norm -> FFN proj -> ...).
// mode 4: same kernel count, no data dependency (all read x_0, summed at the end) --
//   the GPU can overlap independent dispatches, giving the parallel floor.
// mode 5: serial chain with small ops between (GEMV -> RMS_NORM -> GEMV ...) -- the real
//   per-layer pattern where large kernels are separated by dependent small ops.
// serial - parallel delta per kernel = dependency serialization cost of one large kernel.
struct test_op_chain_gemv : public test_case {
    const ggml_type type_w;              // weight type (F16 like real projections)
    const std::array<int64_t, 4> ne_w;   // weight shape [K, N, 1, 1]
    const std::array<int64_t, 4> ne_x;   // activation shape [K, 1, 1, 1]
    const int n_ops;
    const int mode;                      // 3: serial chain; 4: parallel fanout; 5: serial + rms interleaved

    std::string vars() override {
        return VARS_TO_STR4(type_w, ne_w, n_ops, mode);
    }

    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return "OP_CHAIN_GEMV";
    }

    test_op_chain_gemv(ggml_type type_w = GGML_TYPE_F16,
            std::array<int64_t, 4> ne_w = {2048, 2048, 1, 1},
            int n_ops = 8,
            int mode = 3)
        : type_w(type_w), ne_w(ne_w), ne_x({ne_w[0], 1, 1, 1}), n_ops(n_ops), mode(mode) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        std::vector<ggml_tensor *> W(n_ops);
        for (int i = 0; i < n_ops; ++i) {
            W[i] = ggml_new_tensor_4d(ctx, type_w, ne_w[0], ne_w[1], ne_w[2], ne_w[3]);
            char name[64];
            snprintf(name, sizeof(name), "W%d", i);
            ggml_set_name(W[i], name);
        }
        ggml_tensor * x = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, ne_x[0], ne_x[1], ne_x[2], ne_x[3]);
        ggml_set_name(x, "x");

        if (mode == 4) {
            // parallel fanout: all GEMVs read x, sum to force execution
            ggml_tensor * acc = nullptr;
            for (int i = 0; i < n_ops; ++i) {
                ggml_tensor * y = ggml_mul_mat(ctx, W[i], x);
                ggml_set_name(y, "y");
                acc = (acc == nullptr) ? y : ggml_add(ctx, acc, y);
                if (acc != nullptr && i > 0) {
                    char name[64];
                    snprintf(name, sizeof(name), "acc%d", i);
                    ggml_set_name(acc, name);
                }
            }
        ...
        // mode 3: strict serial chain; mode 5: serial with rms between
        ggml_tensor * cur = x;
        for (int i = 0; i < n_ops; ++i) {
            cur = ggml_mul_mat(ctx, W[i], cur);
            char name[64];
            snprintf(name, sizeof(name), "y%d", i);
            ggml_set_name(cur, name);
            if (mode == 5) {
                cur = ggml_rms_norm(ctx, cur, 1e-6f);
                snprintf(name, sizeof(name), "rms%d", i);
                ggml_set_name(cur, name);
            }
        }
    ...
    // Force the memory-based n_runs path (op_size override) instead of flops-based:
    // a GEMV chain's out is MUL_MAT with op_flops > 0, which would pick a huge n_runs
    // and measure a single duplicated GEMV instead of the whole chain.
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ2_S, GGML_TYPE_F32, 128, 8, false, 1408, 1, 2816)); // gemma4 ffn_gate_up fused (K=2816,N=1408,128E,top8)
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ4_NL, GGML_TYPE_F32, 128, 8, false, 2816, 1, 704)); // gemma4 ffn_down (K=704,N=2816,128E,top8)
    // CGC dequant-family probe: same shapes, K-quant direct-nibble vs IQ table vs IQ4_NL
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q2_K,   GGML_TYPE_F32, 256, 8, false,  512, 1, 2048)); // qwen36 gate at Q2_K (direct dequant, ~same bpw as IQ2_S)
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q3_K,   GGML_TYPE_F32, 256, 8, false, 2048, 1,  512)); // qwen36 down at Q3_K (direct dequant, ~same bpw as IQ3_XXS)
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ4_NL, GGML_TYPE_F32, 256, 8, false,  512, 1, 2048)); // qwen36 gate at IQ4_NL (user proposal)
    ...
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q3_K,   GGML_TYPE_F32, 128, 8, false, 2816, 1,  704)); // gemma4 down at Q3_K
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
    ...
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ2_S, GGML_TYPE_F32, 128, 8, false, 1408, 1, 2816)); // gemma4 ffn_gate_up fused (K=2816,N=1408,128E,top8)
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ4_NL, GGML_TYPE_F32, 128, 8, false, 2816, 1, 704)); // gemma4 ffn_down (K=704,N=2816,128E,top8)
    // CGC qwen36 attention probe: SSM layers (30x) + full-attn layers (10x) weight proj, M=1 GEMV
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32, 8192, 1, 2048, {1,1}, {1,1})); // ssm attn_qkv (2048x8192)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32, 4096, 1, 2048, {1,1}, {1,1})); // ssm attn_gate (2048x4096)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32, 2048, 1, 4096, {1,1}, {1,1})); // ssm_out (4096x2048)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32, 8192, 1, 2048, {1,1}, {1,1})); // full attn_q (2048x8192)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32,  512, 1, 2048, {1,1}, {1,1})); // full attn_k (2048x512)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32,  512, 1, 2048, {1,1}, {1,1})); // full attn_v (2048x512)
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_Q6_K, GGML_TYPE_F32, 2048, 1, 4096, {1,1}, {1,1})); // full attn_output (4096x2048)
    test_cases.emplace_back(new test_flash_attn_ext(256, 256, 32, {1,1}, 128, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16)); // SDPA decode M=1, kv=128
    // CGC dequant-family probe: same shapes, K-quant direct-nibble vs IQ table vs IQ4_NL
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q2_K,   GGML_TYPE_F32, 256, 8, false,  512, 1, 2048)); // qwen36 gate at Q2_K (direct dequant, ~same bpw as IQ2_S)
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_Q3_K,   GGML_TYPE_F32, 256, 8, false, 2048, 1,  512)); // qwen36 down at Q3_K (direct dequant, ~same bpw as IQ3_XXS)
    test_cases.emplace_back(new test_mul_mat_id(GGML_TYPE_IQ4_NL, GGML_TYPE_F32, 256, 8, false,  512, 1, 2048)); // qwen36 gate at IQ4_NL (user proposal)
    ...
    //  RMS_NORM 107 / CPY 104 / GLU 67 / SCALE 51 / L2_NORM 51 / SOFT_MAX 34 / DIV 34 /
    //  ARGSORT 33 / SUM_ROWS 33 / CLAMP 33 ...). M=1 real shapes.
    test_cases.emplace_back(new test_rms_norm(GGML_TYPE_F32, {2048, 1, 1, 1}, false, 1e-6f));                 // 107/step
    test_cases.emplace_back(new test_unary(GGML_UNARY_OP_SILU, GGML_TYPE_F32, {2048, 1, 1, 1}));              // 144/step
    test_cases.emplace_back(new test_glu(GGML_GLU_OP_SWIGLU, GGML_TYPE_F32, {2*512, 1, 1, 1}, 0, false));     // 67/step
    test_cases.emplace_back(new test_get_rows(GGML_TYPE_F32, 2048, 1, 8, 1, 1, false));                       // 135/step
    test_cases.emplace_back(new test_soft_max(GGML_TYPE_F32, {256, 1, 1, 1}, false, false, GGML_TYPE_F32, {1, 1}, 1.0f, 0.0f)); // 34/step
    test_cases.emplace_back(new test_argsort(GGML_TYPE_F32, {256, 1, 1, 1}, GGML_SORT_ORDER_DESC));           // 33/step
    test_cases.emplace_back(new test_sum_rows(GGML_TYPE_F32, {2048, 1, 1, 1}, false, false));                 // 33/step
    test_cases.emplace_back(new test_clamp(GGML_TYPE_F32, {2048, 1, 1, 1}, 0.0f, 1.0f));                       // 33/step
    test_cases.emplace_back(new test_scale(GGML_TYPE_F32, {2048, 1, 1, 1}, 1.0f, 0.0f));                       // 51/step
    test_cases.emplace_back(new test_l2_norm(GGML_TYPE_F32, {128, 1, 1, 1}, 1e-6f, false));                    // 51/step

    // CGC bubble-hypothesis probe (S8.65): serial chain of dependent small ops vs the same
    // kernel count with no dependencies, same shape. op_size override forces n_runs=5 so the
    // whole chain is the measured unit. serial-vs-parallel delta isolates GPU dependency
    // latency (identical encode + kernel count); the rms+add alternating case mirrors the
    // real per-layer small-op pattern. Run with CGC_SYNC_GRAPH=1 for synchronous timing.
    test_cases.emplace_back(new test_op_chain(GGML_TYPE_F32, {2048, 1, 1, 1}, 1,  0)); // control: single RMS_NORM
    test_cases.emplace_back(new test_op_chain(GGML_TYPE_F32, {2048, 1, 1, 1}, 16, 0)); // serial chain, 16 RMS_NORM
    test_cases.emplace_back(new test_op_chain(GGML_TYPE_F32, {2048, 1, 1, 1}, 16, 1)); // parallel fanout, 16 RMS_NORM
    test_cases.emplace_back(new test_op_chain(GGML_TYPE_F32, {2048, 1, 1, 1}, 32, 0)); // serial chain, 32 RMS_NORM
    test_cases.emplace_back(new test_op_chain(GGML_TYPE_F32, {2048, 1, 1, 1}, 16, 2)); // serial rms+add alternating (layer pattern)

    // CGC P1-B (S8.70): large-kernel dependency-wait. F16 2048x2048 GEMV chains (M=1),
    // mirroring qwen36 decode projections. mode 3 = serial dependent, mode 4 = parallel
    // fanout (independent), mode 5 = serial + rms interleaved. Run with CGC_SYNC_GRAPH=1.
    test_cases.emplace_back(new test_op_chain_gemv(GGML_TYPE_F16, {2048, 2048, 1, 1}, 1,  3)); // control: single GEMV
    test_cases.emplace_back(new test_op_chain_gemv(GGML_TYPE_F16, {2048, 2048, 1, 1}, 8,  3)); // serial 8 GEMV
    test_cases.emplace_back(new test_op_chain_gemv(GGML_TYPE_F16, {2048, 2048, 1, 1}, 8,  4)); // parallel 8 GEMV
    test_cases.emplace_back(new test_op_chain_gemv(GGML_TYPE_F16, {2048, 2048, 1, 1}, 16, 3)); // serial 16 GEMV
    test_cases.emplace_back(new test_op_chain_gemv(GGML_TYPE_F16, {2048, 2048, 1, 1}, 16, 4)); // parallel 16 GEMV
    test_cases.emplace_back(new test_op_chain_gemv(GGML_TYPE_F16, {2048, 2048, 1, 1}, 8,  5)); // serial 8 GEMV + rms interleaved
