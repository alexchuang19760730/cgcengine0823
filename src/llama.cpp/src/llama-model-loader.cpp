        ...
        if (expert_cache_bytes > 0) {
            ...
            if (strstr(name, "_exps") && strstr(name, "blk.")) {
                ...
                for (int64_t e = 0; e < n_experts; ++e) {
                    ...
                    entry.kind        = kind;
                    entry.type        = (int32_t) cur->type;
                    entry.file_idx    = (uint32_t) weight->idx;
                    entry.file_offset = (uint64_t)(weight->offs + e * expert_bytes);
                    entry.bytes       = (uint64_t) expert_bytes;
                    expert_index.push_back(entry);
                }
            }
        }
