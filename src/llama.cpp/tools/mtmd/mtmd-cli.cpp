...
struct mtmd_cli_context {
    ...
    mtmd_cli_context(common_params & params) : llama_init(common_init_from_params(params)) {
        ...
        // L4 metal pool may cap the context n_batch (expert union must fit pool
        // capacity, §8.88); split image/text chunks at the *actual* context
        // n_batch or decode asserts (n_tokens_all <= cparams.n_batch).
        n_batch = llama_n_batch(lctx);
...
static int eval_message(mtmd_cli_context & ctx, common_chat_msg & msg) {
    ...
    // Reset the media batch from previous message: entries hold raw pointers into
    // the previous message's chunks vector, which was destroyed when that message
    // finished processing. Reusing them would be use-after-free.
    ctx.mbatch.reset();
...
int main(int argc, char ** argv) {
    ...
    // Force simple_io and disable advanced_display when stdout is not a TTY
    // (subprocess mode): avoids readline_advanced echoing stdin back to stdout
    // and ANSI color sequences corrupting response extraction in the wrapper.
    bool is_subprocess = !isatty(STDOUT_FILENO);
    console::init(params.simple_io || is_subprocess, params.use_color && !is_subprocess);
